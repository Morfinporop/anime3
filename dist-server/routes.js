...
import { hashPassword, verifyPassword, signToken, getRandomAvatarColor, requireAuth, requireAdmin, requireUploadPermission, optionalAuth, } from './auth.js';
...
router.post('/auth/register', async (req, res) => {
    try {
        ...
        const r = await query(`INSERT INTO users (username, password_hash, avatar_color, is_admin, can_upload)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, username, avatar_color, is_admin, can_upload`, [username, hash, avatarColor, isAdmin, canUpload]);
        const user = r.rows[0];
        const payload = {
            id: user.id,
...
router.post('/anime', requireAuth, requireUploadPermission, async (req, res) => {
    try {
        ...
        const { rows } = await query(`INSERT INTO anime (title, description, year, genres, poster_data, poster_mime, video_data, video_mime, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
       RETURNING id`, [title, description || '', Number(year) || new Date().getFullYear(), genresArray, posterData, posterMime, videoData, videoMime, req.user.id]);
...
router.get('/files/anime/:id/poster', async (req, res) => {
    try {
        const { rows } = await query(`SELECT poster_data, poster_mime FROM anime WHERE id = $1`, [req.params.id]);
        const anime = rows[0];
...
router.get('/files/anime/:id/video', async (req, res) => {
    try {
        const { rows } = await query(`SELECT video_data, video_mime FROM anime WHERE id = $1`, [req.params.id]);
        const anime = rows[0];
        if (!anime?.video_data) {
...
router.get('/anime/:id/comments', optionalAuth, async (req, res) => {
    ...
    `, [req.params.id]);
        let userLikes = [];
        if (req.user) {
            const likesRes = await query(`SELECT comment_id FROM comment_likes WHERE user_id = $1`, [req.user.id]);
...
router.post('/anime/:id/comments', requireAuth, async (req, res) => {
    try {
        ...
        const { rows } = await query(`INSERT INTO comments (anime_id, user_id, parent_id, text)
       VALUES ($1, $2, $3, $4)
       RETURNING id, created_at`, [req.params.id, req.user.id, parentId || null, text.trim()]);
        res.json({
            id: rows[0].id,
...
router.post('/comments/:id/like', requireAuth, async (req, res) => {
    try {
        ...
        const existing = await query(`SELECT 1 FROM comment_likes WHERE user_id = $1 AND comment_id = $2`, [userId, commentId]);
        ...
        const countRes = await query(`SELECT COUNT(*) as likes FROM comment_likes WHERE comment_id = $1`, [commentId]);
        res.json({
...
router.post('/anime/:id/rate', requireAuth, async (req, res) => {
    try {
        ...
        await query(`INSERT INTO ratings (anime_id, user_id, score)
       VALUES ($1, $2, $3)
       ON CONFLICT (user_id, anime_id) DO UPDATE SET score = $3`, [req.params.id, req.user.id, numScore]);
        const ratingRes = await query(`SELECT COALESCE(AVG(score), 0) as avg, COUNT(*) as count FROM ratings WHERE anime_id = $1`, [req.params.id]);
        res.json({
...
router.get('/admin/users', requireAuth, requireAdmin, async (_req, res) => {
    try {
        const { rows } = await query(`SELECT id, username, avatar_color, is_admin, can_upload, created_at 
