...
export default function AdminPage({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => { if (user?.isAdmin) refreshUsers(); }, [user, refreshUsers]);
  useEffect(() => { if (tab === 'anime') api.getAnimeList().then(setAnimeList).catch(() => {}); }, [tab]);

  if (!user?.isAdmin) {
    return (
      <div className="px-4 py-6">
        <div className="flex items-center justify-between mb-4"><h2 className="text-xl font-bold text-zinc-900">Админ-панель</h2>
          <button onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-full text-zinc-400 hover:bg-zinc-100"><X className="h-4 w-4" /></button></div>
    ...
    <div className="px-4 py-6">
              ...
              <tbody>
                {filteredUsers.map(u => (
                  <tr key={u.id} className="border-b border-zinc-100 last:border-0 hover:bg-zinc-50/50">
                    <td className="px-3 py-2.5"><div className="flex items-center gap-2"><div className="flex h-7 w-7 items-center justify-center rounded-full text-[10px] font-bold text-white flex-shrink-0 overflow-hidden" style={(u as any).avatarData ? {} : { backgroundColor: u.color }}>{(u as any).avatarData ? <img src={(u as any).avatarData} alt="" className="h-full w-full object-cover" /> : u.nickname.charAt(0).toUpperCase()}</div><div><div className="font-semibold text-zinc-900 text-xs">{u.nickname}</div><div className="text-[10px] text-zinc-500 sm:hidden">ID: {u.id}</div></div></div></td>
                    ...
                    <td className="px-3 py-2.5"><div className="flex justify-end gap-1">
                      <button onClick={() => toggleAdmin(u.id)} disabled={u.id === user.id} className={`flex h-7 items-center rounded-full px-2.5 text-[10px] font-medium ${u.isAdmin ? 'bg-pink-100 text-pink-700' : 'border border-zinc-200 text-zinc-600'} disabled:opacity-30`}>Админ</button>
                      <button onClick={() => toggleUpload(u.id)} disabled={u.isAdmin} className={`flex h-7 items-center rounded-full px-2.5 text-[10px] font-medium ${u.canUpload && !u.isAdmin ? 'bg-emerald-100 text-emerald-700' : 'border border-zinc-200 text-zinc-600'} disabled:opacity-30`}>Загрузка</button>
                      <button onClick={() => { if (confirm(`Удалить ${u.nickname}?`)) deleteUser(u.id); }} disabled={u.isAdmin} className="flex h-7 w-7 items-center justify-center rounded-full text-zinc-400 hover:text-red-500 disabled:opacity-30"><Trash2 className="h-3.5 w-3.5" /></button>
                    </div></td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === 'anime' && (
        <div>
          ...
          <input type="text" value={animeSearch} onChange={(e) => setAnimeSearch(e.target.value)} placeholder="Поиск по названию или ID..." className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2 text-sm outline-none focus:border-zinc-400 mb-3" />
          <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white">
            <table className="w-full text-sm"><thead className="border-b border-zinc-200 bg-zinc-50"><tr className="text-left text-xs font-bold uppercase tracking-wider text-zinc-500"><th className="px-3 py-2.5">Название</th><th className="hidden px-3 py-2.5 sm:table-cell">ID</th><th className="px-3 py-2.5 text-right">Действия</th></tr></thead>
              <tbody>
                {filteredAnime.map((a: any) => (
