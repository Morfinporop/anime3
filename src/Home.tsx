...
export default function HomePage() {
  ...
  // Собираем жанры динамически из загруженных аниме
  useEffect(() => {
    const usedGenres = new Set<string>();
    animeList.forEach((a: any) => {
      if (Array.isArray(a.genres)) a.genres.forEach((g: string) => usedGenres.add(g));
    });
  ...
  if (genre !== 'all') {
    filteredList = filteredList.filter(a => a.genres?.includes(genre));
  }
  ...
  const cardsWithData: CardData[] = filteredList.map((a: any) => ({
    id: a.id,
    title: a.title,
  ...
  return (
    <div>
      ...
      <section className="relative overflow-hidden border-b border-zinc-200">
        <img 
          src="https://avatanplus.com/files/resources/original/5cc1631bacf9316a536b243d.png" 
          alt="" 
        ...
        <div className="relative mx-auto max-w-[1400px] px-5 py-10 sm:px-8 sm:py-16">
          ...
          <p className="mt-3 max-w-xl text-sm text-white/90 sm:text-base leading-relaxed">
            ...
            Мы создали этот большой, удобный, детально проработанный сборник, чтобы каждый 
            зритель легко находил новые релизы, популярные многосерийные тайтлы и редкие 
            ретро-картины для идеального вечернего просмотра.
          </p>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1400px] px-5 pb-12 sm:px-8 sm:pb-16">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-full border border-zinc-200 bg-white p-1">
              <button 
                ...
                className={`rounded-full p-1.5 ${view === 'grid' ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`} 
                aria-label="Карточки"
              >
              ...
              <button 
                ...
                className={`rounded-full p-1.5 ${view === 'rows' ? 'bg-zinc-900 text-white' : 'text-zinc-500'}`} 
                aria-label="Полоски"
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6">
            {cardsWithData.map((v) => (
              <div key={v.id} onClick={() => handleCardClick(animeList.find(a => a.id === v.id) || v)} className="cursor-pointer">
                <Card data={v} />
              </div>
            ))}
          </div>
